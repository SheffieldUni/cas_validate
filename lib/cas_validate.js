...
function json_ticket(options)
{
    ...
    var cas_host = force_protocol(options.cas_host);
    if (! cas_host ) throw new Error('no CAS host specified');
    var opt_service = options.service;
    ...
    return function (req, res, next)
    {
        ...
        req.session.ticket = url.query.ticket;
        logger.info('Checking ticket ' + req.session.ticket);
...
function ticket(options){
    ...
    if (! cas_host ) throw new Error('no CAS host specified');
    var opt_service = options.service; // for example: 'http://safety.ctmlabs.net/geojson';
    var isJson = options.json;
    var jsonValidateUrl = options.jsonUrl;
    ...
    return function(req,res,next){
        ...
        // The CAS plugin to support Json ticket validation normally resides at the default cas url.
        // It doesn't seem to do content negotiation so to
        var validation_uri = options.json && options.jsonValidateUri !== undefined ? options.jsonValidateUri : validation_service;
            ...
            if (!error && response.statusCode == 200) {
                ...
                if(options.json)
                {
                    ...
                    if(/cas:authenticationSuccess/.exec(body)){
                        ...
                        next(new Error('CAS ticket validation failed'));
                    }
